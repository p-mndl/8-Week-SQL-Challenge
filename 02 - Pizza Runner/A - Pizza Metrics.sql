...
SELECT
    ro.runner_id,
    COUNT(ro.pickup_time) AS succesful_orders
...
SELECT
    ...
    cnt_vegetarian =
    COUNT(CASE WHEN pizza_names.pizza_name = 'Vegetarian' THEN 1 END),
    cnt_meatlover = COUNT(
        CASE
...
WITH pizza_per_order AS (
    SELECT
        ...
        rank_pizza_count = DENSE_RANK() OVER (
...
SELECT cnt_exclusions_extras = COUNT(ee.changes)
FROM extras_exclusions AS ee
LEFT JOIN cs2.runner_orders AS ro
    ON ee.order_id = ro.order_id
WHERE ro.cancellation IS NULL;
...
--einfacher
SELECT cnt_exclusions_extras = COUNT(*)
FROM cs2.customer_orders AS co
LEFT JOIN cs2.runner_orders AS ro
    ON co.order_id = ro.order_id
WHERE
