...
WITH ranked_orders AS (
	SELECT s.customer_id, men.product_name, DENSE_RANK() OVER (PARTITION BY s.customer_id ORDER BY s.order_date) AS order_rank
...
SELECT men.product_name, COUNT(s.product_id) AS purchase_count
FROM dannys_diner.sales AS s
INNER JOIN dannys_diner.menu AS men
    ON s.product_id = men.product_id
...
SELECT ranked_sales_membership.customer_id
